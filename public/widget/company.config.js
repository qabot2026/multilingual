 ...
 * - **Single JS bundle:** run `python scripts/build_widget_bundle.py` (writes `dist/company-widget.bundle.js` + `dist/company.css`).
 *   Use `static/company-loader.example.js` as the client one-script embed (set `WIDGET_BASE_URL`, host on your CDN). Edit `static/*`, then rebuild.
 *
...
/**
 * Optional: map the embed script query `?botid=` (loader sets `window.__DFCHAT_BOT_ID__`) to Dialogflow ids.
 * Add one row per client bot. Keys are strings, e.g. "0001".
 */
window.DFCHAT_BOT_MAP = {
  "0001": {
    projectId: "qabot01",
    location: "us-central1",
    agentId: "05ce7add-9025-4534-990c-fd7a25dadde1"
...
(function applyDfBotIdFromMap() {
  if (typeof window === "undefined") {
    return;
  }
  const bid = window.__DFCHAT_BOT_ID__;
  const map = window.DFCHAT_BOT_MAP;
  if (bid == null || bid === "" || !map || typeof map !== "object") {
    return;
  }
  const row = map[String(bid)];
  if (!row || typeof row !== "object") {
    return;
  }
  const c = window.COMPANY_CHAT_UI_CONFIG;
  if (!c || !c.common || !c.common.dialogflow) {
    return;
  }
  const df = c.common.dialogflow;
  if (typeof row.projectId === "string" && row.projectId) {
    df.projectId = row.projectId;
  }
  if (typeof row.location === "string" && row.location) {
    df.location = row.location;
  }
  if (typeof row.agentId === "string" && row.agentId) {
    df.agentId = row.agentId;
  }
})();

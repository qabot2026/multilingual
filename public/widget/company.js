...
function createAndMountMessenger() {
    ...
    scheduleTitlebarRoundCornersToMessenger(df);
    ...
    [200, 500, 1200, 2500, 4000].forEach((ms) => {
        window.setTimeout(() => {
            ...
            applyTitlebarRoundCornersToMessenger(df);
        }, ms);
    });
...
function applyDfMessengerThemeConfig(dfMessenger, config) {
    ...
    scheduleTitlebarRoundCornersToMessenger(dfMessenger);
}
 ...
 * @returns {void}
 */
function applyTitlebarRoundCornersToMessenger(dfMessenger) {
    if (!dfMessenger || typeof dfMessenger.querySelector !== "function") {
        return;
    }
    ...
    for (const root of roots) {
        ...
        let list;
        try {
            list = root.querySelectorAll("df-messenger-header");
        } catch (e) {
            continue;
        }
        for (const header of list) {
            ...
            if (header && header.shadowRoot) {
                ...
                if (!tag) {
                    tag = document.createElement("style");
                    tag.id = TITLEBAR_ROUND_CORNERS_STYLE_ID;
                    header.shadowRoot.appendChild(tag);
                }
                if (tag.textContent !== css) {
                    tag.textContent = css;
                }
 ...
 * @returns {void}
 */
function scheduleTitlebarRoundCornersToMessenger(dfMessenger) {
    if (!dfMessenger) {
        return;
    }
    const run = () => {
...
function restartChatSession() {
    closeForm();
    // Drop a body-mounted Language/Restart bar so `ensureChatActionBar` creates a new one. Reusing a bar that was
    // `display: none` while the panel was torn down can leave the wrong state on slow/embed hosts (e.g. GitHub Pages).
    {
        const prevBar = document.getElementById(CHAT_ACTION_BAR_ID);
        if (prevBar) {
            try {
                prevBar.remove();
            } catch {
    ...
    if (m) {
        const resyncActionBar = () => {
            if (activeDfMessenger !== m) {
                return;
            }
            ensureChatActionBar();
            scheduleSyncChatActionBarPosition();
        };
        // Footer/shadow is often ready a bit later on embedded + CDN; these retries match createAndMountMessenger’s 120ms nudge.
        [80, 320, 900, 2200].forEach((d) => {
            window.setTimeout(resyncActionBar, d);
        });
    }
}
